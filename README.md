...
## Shiny semantic explorer (documentation + mobile-friendly HTML)
...
These plots are generated by `Rscript analysis/plot_gc_chunk_embed_results.R` after the chunk-embedding pipeline runs; copies for the Shiny app live under `analysis/shiny_gc_family/www/`.
...
- **Interactive explorer:** `analysis/shiny_gc_family/` — see `analysis/shiny_gc_family/HOW_TO_RUN.md` and [`documentation/shiny-family-explorer.md`](documentation/shiny-family-explorer.md). Bundled PNGs in `www/` match the files above; the **Chunk insights** tab uses `data/chunk_highlights.rds` when present (built by the same plot script if `chunks_scored.parquet` exists).
